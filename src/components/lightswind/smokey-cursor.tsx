...
export default function SmokeyCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      if (!formatRG) {
        formatRG = isWebGL2 && (gl as WebGL2RenderingContext).RG
          ? { internalFormat: (gl as WebGL2RenderingContext).RG, format: (gl as WebGL2RenderingContext).RG }
          : { internalFormat: gl.RGBA, format: gl.RGBA };
      }
      if (!formatR) {
        formatR = isWebGL2 && (gl as WebGL2RenderingContext).RED
          ? { internalFormat: (gl as WebGL2RenderingContext).RED, format: (gl as WebGL2RenderingContext).RED }
          : { internalFormat: gl.RGBA, format: gl.RGBA };
      }
      ...
      return {
        ...
        ext: {
          ...
          formatRG,
          formatR,
          halfFloatTexType: halfFloatTexType || gl.UNSIGNED_BYTE,
    ...
    function generateColor(): ColorRGB {
      // Elegant cool palette: cyan (0.50), blue (0.58), violet (0.70), subtle purple (0.76)
      const hue = 0.50 + Math.random() * 0.26;
    ...
    const getCoords = (clientX: number, clientY: number) => {
      if (!canvas) return { x: scaleByPixelRatio(clientX), y: scaleByPixelRatio(clientY) };
